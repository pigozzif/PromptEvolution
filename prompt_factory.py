...
class PromptFactory(object):
    DEMO = "I instruct my friend to <mask>\nThe friend read the instruction and wrote an output for every one of " \
           "the inputs. Here are the input-output pairs:"
    ...
    def _fill_demo(self):
        self.DEMO = "\n".join([self.DEMO] + ["Input {0} Output {1}".format(i, o[0])
                                             for i, o in zip(self.test_data[0], self.test_data[1])]).lower()
